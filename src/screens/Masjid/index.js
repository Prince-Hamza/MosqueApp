...
console.log("coords: " + coords)
            ...
            locationService.getNearbyMasjids(coords.latitude, coords.longitude).then(res => {
              console.log("res : " + res)
    ...
    const getListOfMasjid = () => {
      if (this.state.masjids.length > 0) {
        return this.state.masjids.map((masjid, index) => {
          const ref = (masjid.photos && masjid.photos.length) > 0 ? masjid.photos[0].photo_reference : null
          ...
          return (
            <ListItem avatar key={index} style={{ marginTop: 5, marginBottom: 10 }}>
              <Left>
                {ref != null &&
                  <Thumbnail square source={{ uri: `https://maps.googleapis.com/maps/api/place/photo?maxwidth=200&photoreference=${ref}&key=AIzaSyDDz5dBxtVXpeUEBtEdQA0r6fqnHniM8FA` }} />
                }
                {ref === null &&
              ...
              <Body>
                ...
                <Text note><Icon name='md-locate' style={{fontSize: 20, color: colors.primaryColor, marginTop: 15, marginRight: 10}} /> {fixDistance} km</Text>
              </Body>
              <Right>
                <Button transparent onPress={() => this.goToMap(masjidLat, masjidLong)}>
                  <Icon name='md-navigate' style={{fontSize: 30, color: colors.primaryColor}} />
                </Button>
              </Right>
            </ListItem>
          )
          ...
          <View style={{ flex: 1}}>
            {!this.state.isLoading &&
              <MapView
                ...
                style={{ height: 300 }}
              >
                {(!this.state.isListLoading && this.state.masjids.length > 0 ) && this.state.masjids.map((masjid, i) => (
                  <Marker
                    key={i}
                    coordinate={{
